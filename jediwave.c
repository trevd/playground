...
/*	The Jediwave is a modified version of the sdcard binary / service
	which is technically a low level fuse driver implementation

	When Executed it will attempt to mount the directory /system/.sbin
	to /system/sbin. /system/sbin is included in the PATH environment
	variable on most android devices but is seldom used and often 
	doesn't exist at all. 
	
	You can limit access to this directory by setting or adding this 
	property this property to the build.prop/default.prop 
	not.the.droid=<application user id> 
	
	The application user id can be found in /data/system/packages.xml
	This will prevent the specific application from being able to read 
	the contents of the directory while all other applications function 
	normally
	
	The majority of fuse request are logged to the logcat and file access 
	attempts are displayed should an application scan the usual executable 
	paths for any reason
	
	Setup : Rooted Device! ( obviously ) 
	Use adb or your favourite terminal emulator
	Remount the system as r/w and create /system/.sbin and /system/sbin
	Make sure you have fuse enable in the kernel and /dev/fuse exists
	Set the not.the.droid property as required or just leave blank for 
	monitoring
	
	You can adjust SOURCE_PATH DEST_PATH and recompile to change
	the mount points.
	
	Note: At the moment the directory is mounted as a fat32 partition 
	which obviously has limited file attribute ( i.e no exe flag )
	
	Note 2: If you somehow manage to break your device using this code then
	you should go and tell someone who gives a fuck.
	

*/

#define SOURCE_PATH "/system/.sbin"
#define DEST_PATH "/system/sbin"
...
// Log to the logcat for now
#if FUSE_TRACE
#define TRACE(x...) ALOGD(x) ; ALOGD("user id:%d group id:%d\n",getuid(),getgid()); fprintf(stderr,x); fprintf(stderr,"pid id:%d group id:%d\n",getpid(),getegid());
#else
        ...
        const struct fuse_in_header* hdr, const struct fuse_read_in* req)
{
  ...
  TRACE("[%d] READDIR hdr->uid=%d\n", handler->token , hdr->uid);
    TRACE("[%d] READDIR %p\n", handler->token, h);
    if (req->offset == 0) {
...
{
    int res;
    const char *source_path = SOURCE_PATH ; // NULL;
    const char *dest_path = DEST_PATH ; // NULL;
    uid_t uid = 0;
    gid_t gid = 0;
    int num_threads = DEFAULT_NUM_THREADS;
    int i;
    //FILE * fopen("/system/.sbin/sky","r");
